...
class VeyronisRepository(private val database: AppDatabase) {
    ...
    suspend fun deleteCharacter(character: Character) = database.characterDao().deleteCharacter(character)
    ...
    suspend fun deleteCodexEntry(entry: CodexEntry) = database.codexDao().deleteEntry(entry)
    ...
    suspend fun deleteStoryEvent(event: StoryEvent) = database.storyEventDao().deleteEvent(event)
